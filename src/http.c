...
/* Ignore SIGPIPE so we don't die while doing this */
    ...
    if(response == NULL || (http->response = (char*)malloc(len+1)) == NULL)
        return(1);
    strncpy(http->response, response, len);
    do
    http->response[len--] = '\0';
    while(len > 0 && (http->response[len] == '\r' || http->response[len] == '\n'));
...
http_parse_headers(HTTP *http)
{
    ...
    while((getline(&ll, &len, f)) > 0){


        line = l = (char*)malloc(len+1);
        l[len] = '\0';
        ...
        //node = ;
        node = (HDRNODE*)malloc(sizeof(HDRNODE*));
        if(notFirst){
        	env = node;
        	notFirst = 0;
        }
	    ...
	    //set the next char after l to null
	    *cp++ = '\0';
