...
void print_url(URL* url){
    printf("Method: %s | Hostname: %s | Port: %d | Path: %s \n", url->method, url->hostname, url->port, url->path);
...
*/      
        if(colon < slash) {
            *colon = '\0';
            ...
            cp = colon+1;
            if(!strcasecmp(up->method, "http")){//returns 0 if strings are equal..
                up->port = 80;
            }
        }
        //If the that slash+1 is pointing to is a slash, so a double slash
        if(*(slash+1) == '/') {
/*
